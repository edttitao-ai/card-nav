...
function Sidebar({ isOpen, onClose, activeItem, onSelectItem }) {
  return (
    <>
      {isOpen && (
        <div
      ...
      >
        <div className="h-16 flex items-center justify-between px-5" style={{ borderBottom: '1px solid #ede9e1' }}>
          <div className="flex items-center gap-3">
            ...
            >
              <svg className="w-4 h-4" style={{ color: '#c0612a' }} viewBox="0 0 16 16" fill="none">
                <path d="M2 8h12M8 2v12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
              </svg>
            </div>
            <span className="text-sm font-semibold tracking-tight" style={{ color: '#3d3831' }}>
              导航站
            </span>
          </div>
          <button
            ...
            {SIDEBAR_ITEMS.map(item => (
              <li key={item.id}>
                <button
                  ...
                  style={{
                    background: activeItem === item.id ? 'rgba(192, 97, 42, 0.08)' : 'transparent',
                    color: activeItem === item.id ? '#c0612a' : '#5c5049',
                    fontWeight: activeItem === item.id ? 500 : 400,
                  }}
                >
                  <Icon name={item.id} className="w-5 h-5 shrink-0" />
        ...
        <div className="p-4" style={{ borderTop: '1px solid #ede9e1' }}>
          <div className="text-xs" style={{ color: '#c9c0b4' }}>
            v1.0.0
          </div>
        </div>
      </aside>
...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      setCards(DATA_MAP[activeSidebarItem] || [])
    }
    setSearch('')
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY_PREFIX + activeSidebarItem, JSON.stringify(cards))
    } catch (e) {
      console.warn('localStorage save failed:', e)
    }
  }, [cards, activeSidebarItem])

  const categories = useMemo(() => ['全部', ...new Set(cards.map(c => c.category))], [cards])
  ...
  const currentSidebarItem = SIDEBAR_ITEMS.find(i => i.id === activeSidebarItem)
  ...
  return (
    <div className="min-h-screen flex" onContextMenu={e => {
      ...
      <div className="flex-1 min-w-0">
        <header
          className="lg:hidden sticky top-0 z-30 h-14 flex items-center px-4"
          style={{
