  ...
  return (
    <div className="p-5 rounded-2xl" style={{ background: '#ffffff', border: '1px solid #ede9e1' }}>
      <div className="flex items-start justify-between mb-3">
        <span className="text-base font-bold" style={{ color: '#3d3831' }}>{title}</span>
        {trend !== undefined && (
      ...
      <div className="text-4xl font-bold tracking-tight mb-1" style={{ color: accent ? '#c0612a' : '#3d3831' }}>
        {value}
      </div>
      {subtitle && (
        <p className="text-sm" style={{ color: '#8c7e72' }}>{subtitle}</p>
      )}
    </div>
  )
}